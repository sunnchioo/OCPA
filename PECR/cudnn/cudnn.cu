...
int main(int argc, char *argv[])
{
    ...
    fstream open_kernel("/home/lfa/fsy/syt/conv_pool/dataset/vggdata/kernel_name.txt");
    for (int i = 0; i < 16; i++)
    {
    ...
    ofstream time_file(string("/home/lfa/fsy/syt/conv_pool/PECR/cudnn/time/time_batchsize") + argv[1] + string(".txt"));
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        cudaMemcpy(in_data, matrix, in_n * in_c * in_h * in_w * sizeof(float), cudaMemcpyHostToDevice);
        ...
        cudaMalloc(&gpu_out, (out_h / 2) * (out_h / 2) * sizeof(float));
        ...
        cudaMemcpy(result, gpu_out, result_size * sizeof(float), cudaMemcpyDeviceToHost);
